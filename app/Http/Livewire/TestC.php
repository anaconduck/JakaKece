...
class TestC extends Component
{
    ...
    {
        if (!strpos(url()->previous(), 'inkubasi' ) and (!strpos(url()->previous(), "test/$course"))) {
        ...
        if ($historyTest == null or $historyTest->status_selesai) {
            $new = HistoryTest::makeHistoryTest(auth()->user()->identity, $course, $this->sesiSoal, $this->sesi);
            $historyTest = $new[0];
            ...
            if ($waktu >= config('app.totalTimeTest')[$course]) {
                ...
                $new = HistoryTest::makeHistoryTest(auth()->user()->identity, $course, $this->sesiSoal);
                $historyTest = $new[0];
                $historyJawaban = $new[1];
                $this->sesi = 0;
            } else {
                ...
                if (!$historyJawaban) {
                    $historyJawaban = HistoryJawabanTest::makeHistoryJawaban($historyTest->id, $this->sesi, min($this->allsesi[$this->sesi]['num'], Test::countQuest($this->idCourse, $this->sesiSoal)));
                }
                if ($this->sesi == 0) {
        ...
        $soal = Test::show($this->idCourse, $this->sesiSoal, $this->daftarSoal[$this->posisiSoal])->toArray();
        if (!($soal == null))
            $this->soal = $soal;
        else abort(500);
    ...
    public function show($soal)
    {
        if ($soal >= 0 and $soal < $this->allsesi[$this->sesi]['num']) {
            ...
            $soal = Test::show($this->idCourse, $this->sesi, $this->daftarSoal[$this->posisiSoal])->toArray();
            if (!($soal == null))
                $this->soal = $soal;
            else abort(500);
            if ($this->soal['tipe'] == 'f')
