                        ...
                        <div class="col-md-12" style="border-left: 5px solid rgba(0, 0, 0, 0.075)">
                            @if ($riwayat)
                                <div class="row">
                                    @foreach ($riwayat as $data)
                                        ...
                                        <a class="card2 col-sm-4" href="{{ url("/user/riwayat-latihan/$data->id") }}">
                                            <h3>{{ config('app.allCourse.' . $data->id_course) ?? '-' }}</h3>
                                            <p class="small">Status :
                                                @switch($data->status_selesai)
                                                    @case(0)
                                                        Belum Selesai
                                                    @break
                                                    @case(1)
                                            ...
                                            <p class="small">
                                                Tanggal mulai : {{ date("d M Y", strtotime($data->created_at)) }}
                                            </p>
                            ...
                            @else
                                <div class="container ex">
                                    <h3>Anda belum pernah mengikuti sesi latihan di Inkubasi Bahasa. <br><a
                                            href="{{ url('/inkubasi') }}">Ikut sesi latihan!</a></h3>
