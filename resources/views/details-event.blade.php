        ...
        <div class="col-sm-9 right">
            <article class="content">
                ...
                <dl class="row">
                    ...
                    <dd class="col-sm-8">{{ date('d / M / Y',strtotime($event->mulai_daftar)) }}</dd>

                    <dt class="col-sm-4">Tanggal Akhir Pendaftaran</dt>
                    <dd class="col-sm-8">{{ date('d / M / Y',strtotime($event->akhir_daftar)) }}</dd>

                    <dt class="col-sm-4">Tanggal Event</dt>
                    <dd class="col-sm-8">{{ date('d / M / Y',strtotime($event->mulai)) }}</dd>

                    <dt class="col-sm-4">Jumlah Anggota</dt>
                    <dd class="col-sm-8">{{ $event->max_anggota}}</dd>

                    @if($event->file)
                    <dt class="col-sm-4">Poster</dt>
                    <dd class="col-sm-8"><a style="color: rgb(73, 152, 255);" href="{{ asset($event->file)}}"> lihat poster </a></dd>
                    @endif

                    @if($terlaksana)
                    <dt class="col-sm-4">Jumlah Pendaftar</dt>
                    <dd class="col-sm-8">{{ $numPendaftar ??0}}</dd>
                    @endif

                    @if($event->finish)
                    <dt class="col-sm-4">Jumlah Pemenang</dt>
                    <dd class="col-sm-8">{{ $numPemenang ??0}}</dd>
                    @endif
