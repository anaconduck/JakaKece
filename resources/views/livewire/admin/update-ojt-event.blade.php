            ...
            <div class="row">
                <div class="col-md-12">
                    <div wire:ignore class="section-heading">
                        <h2>Update Ojt - {{ $event->nama_event }}</h2>
                        @if ($errors->any())
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <fieldset>
                                    ...
                                    <input wire:model.lazy="namaEvent" id="nama" value="{{ $event->nama_event }}"
                                        placeholder="Nama Event" class="form-control file" type="text" name="nama_event">
                                </fieldset>
                            </div>
                            <div class="col-md-6">
                                <fieldset>
                                    <label for="nama">Max Konversi SKS</label>
                                    <input wire:model.lazy="max" id="nama" value="{{ $max }}"
                                        placeholder="max konversi" class="form-control file" type="number" name="nama_event">
                                </fieldset>
                            </div>
                            <div class="col-md-12">
                                <label for="dalam_negeri">Prodi</label>
                                <select wire:model.lazy="prodi" name="prodi" id="prodi" required>
                                    @foreach (config('app.prodi') as $ind => $item)
                                        <option value="{{ $ind }}" @if ($prodi == $ind) selected @endif>{{ $item }}</option>
                                    @endforeach
                                </select>
                            </div>
                                    ...
                                    <tbody>
                                        @foreach ($mk as $m)
                                            <tr>
                                                ...
                                                <td>{{ $m['sks'] ?? 0 }}</td>
                                                <td>
                                                    <button type="button" wire:click="updateMk({{ $m['id'] }})"
                                        ...
                                        <tr>
                                            <td colspan="4">
                                                <button wire:click="tambahMK" id="tambahmk" type="button"
                                                    class="btn btn-primary" da ta-toggle="modal"
                    ...
                    <div class="modal-body">
                        @foreach ($listMK as $m)
                            <label class="toggle" for="{{ $m['id'] }}">
                                <input wire:click="updateMk({{ $m['id'] }})" name="mk_{{ $m['id'] }}"
                                    type="checkbox" class="toggle__input" id="{{ $m['id'] }}"
                                    value="{{ $m['id'] }}" @if (in_array($m['id'], $idMk))
                                checked
                        @endif>
...
<div class="modal-footer">
    <button wire:click="closeTambah" type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
