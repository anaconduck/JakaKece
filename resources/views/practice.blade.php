                            ...
                            <div class="card quest">
                                ...
                                <div class="card-body">
                                        ...
                                        @if ($quest)
                                            ...
                                            <div class="input-group mb-3">
                                                <div class="input-group-text">
                                                    @if(session("jawaban.$type/$kategori/$id")== $opsi)
                                                    <input class="form-check-input mt-0" type="radio" value="{{ $opsi }}" id="{{ $opsi }}" aria-label="Checkbox for answer" name="answer" checked>
                                                    @else
                                                    <input class="form-check-input mt-0" type="radio" value="{{ $opsi }}" id="{{ $opsi }}" aria-label="Checkbox for answer" name="answer">
                                                    @endif
                                                </div>
                                                <label for="{{ $opsi }}">
                                                    {{ $opsi }}
                                  ...
                                  <div class="row text-center">
                                      <div class="col-6">
                                        <a href="{{ $id>1?url("/latihan/$type/$kategori/".($id-1)):url("/latihan/$type/$kategori/$id") }}" class="btn">Prev</a>
                                      </div>
                                      <div class="col-6">
                                        <a href="{{ $id<$num?url("/latihan/$type/$kategori/".($id+1)):url("/latihan/$type/listening/pre") }}" class="btn">{{ $id<$num?'Next':'Next Section' }}</a>
                                      </div>
                                  </div>
                                </div>
                              </div>
